...
*  这题的关键在于及时确定奇偶，我们可以将a[i] 分成奇偶两组，显然我们只要从一端开始，就可以明确变化后的数组
  * 显然，对于全奇或全偶的数组，我们可以在 n-1次实现，每次都是 [i,n];
  * 我们可以先将一组的统一，接下来就剩：奇数+ 偶数=奇数（改动右边），于是我们想要用1作为每次的l,
    * 显然，我们将 a[1]分奇偶讨论，先将a[1]所在组实现统一即可
